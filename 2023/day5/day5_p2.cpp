...
//  tuples of ranges
// for range, check if source is in range, for source <= x <= end of range, transform then move to next mapping section in a new vector. 
// for below the source, check the next mapping in the section
// key difference is to maintain a vector of vectors for the current mapping section rather than iterating through with each seed. 
