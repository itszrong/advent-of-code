...
fn main() {
    ...
    let xcoord1 = coord1.split(',').map(str::trim).map(str::parse::<f64>).collect::<Result<Vec<_>, _>>().unwrap()[0];
    let coord2 = contents.lines().collect::<Vec<_>>()[pair.2];
    let xcoord2 = coord2.split(',').map(str::trim).map(str::parse::<f64>).collect::<Result<Vec<_>, _>>().unwrap()[0];
    println!("{:?}", (xcoord1*xcoord2) as i64);
}
