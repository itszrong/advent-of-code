...
# print(len(dot_indices))

# def run_simulation(input_df_with_obs):
#     loop = False
#     seen_states = set()
#     step = 0
#     while any(item in directions.keys() for item in input_df_with_obs.values.flatten()):
#         for char in directions.keys():
#             if char in input_df_with_obs.values:
#                 # display(input_df_with_obs)
#                 index = (input_df_with_obs.stack() == char).idxmax()
#                 input_df_with_obs.iloc[index] = "X"
#                 if (0 <= index[0] + directions[char][0] < input_df_with_obs.shape[0]) and (0 <= index[1] + directions[char][1] < input_df_with_obs.shape[1]):
#                       if (0 <= index[0] + 2* directions[char][0] < input_df_with_obs.shape[0]) and (0 <= index[1] + 2* directions[char][1] < input_df_with_obs.shape[1]):
#                           if input_df_with_obs.iloc[index[0] + 2 * directions[char][0], index[1] + 2 * directions[char][1]] in ["#", "O"]:
#                               new_char = get_next_element(directions_keys, char)
#                               # print(new_char)
#                               input_df_with_obs.iloc[index[0] + directions[char][0], index[1] + directions[char][1]] = new_char
#                           else:
#                               input_df_with_obs.iloc[index[0] + directions[char][0], index[1] + directions[char][1]] = char
#                       else:
#                           input_df_with_obs.iloc[index[0] + directions[char][0], index[1] + directions[char][1]] = char
#                 # Break the inner loop after processing a character
#                 current_state = tuple(input_df_with_obs.values.flatten())
...
#     print(i)
#     # print(obstacle_step)
#     input_df_with_obs = copy_df.copy()
#     input_df_with_obs.iloc[dot_index] = "O"  # Changed from "O" to "O"
#     result, loop = run_simulation(input_df_with_obs)
#     display(result)
#     x_count = (result == 'X').sum().sum()
#     if x_count != 41:
#         print(f"The number of 'X' characters in the DataFrame is: {x_count}")
#     if loop == True:
#       no_of_obstacles_loops += 1
#     obstacle_step += 1
# print(no_of_obstacles_loops)
